...
| `ondewo-t2s-api`, `ondewo-proto-compiler` | their own repositories | as submodule commits |
...
1. Bump `ONDEWO_T2S_API_GIT_BRANCH` (and, for a new compiler release,
...
Releases are cut from the `Makefile`, never by hand: bump `ONDEWO_T2S_VERSION` (it must match the
ONDEWO T2S API in major and minor version), add a `RELEASE.md` entry under a
`## Release ONDEWO T2S PHP Client <version>` heading followed by the `*****` separator the release
