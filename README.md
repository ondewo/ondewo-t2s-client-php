<div align="center">
  ...
  <h1>
  ONDEWO T2S Client PHP Library
  </h1>
</div>

This library is the PHP gRPC client for the **ONDEWO T2S** (Text-to-Speech) server.
...
[ondewo-t2s-api](https://github.com/ondewo/ondewo-t2s-api) repository by the
...
```bash
composer require ondewo/t2s-client-php
```

To work on the client itself:

```bash
git clone --recurse-submodules git@github.com:ondewo/ondewo-t2s-client-php.git
cd ondewo-t2s-client-php
...
```
.
├── ondewo-t2s-api          <----- submodule: the .proto definitions (ondewo/ = the services, google/ = imports)
...
docker run --rm \
  -v $(pwd):/input-volume \
  -v $(pwd):/output-volume \
  ondewo-php-proto-compiler:latest ondewo-t2s-api ondewo
```
...
use Ondewo\T2s\SynthesizeRequest;
use Ondewo\T2s\Text2SpeechClient;

// The PHP namespace is protoc's UpperCamel form of the proto package:
// `package ondewo.t2s;` becomes `Ondewo\T2s`, and the service `Text2Speech` becomes
// `Text2SpeechClient` (grpc_php_plugin's default class suffix). Browse src/Ondewo/T2s for
...
$client = new Text2SpeechClient(
    getenv('ONDEWO_T2S_HOST') ?: 'localhost:50055',
...
$request->setText('Hallo Welt');
$request->setConfig($config);

[$response, $status] = $client->Synthesize($request)->wait();
...
make test      # the same, plus `make check_build` against the ondewo-t2s-api submodule
```
...
`ONDEWO_T2S_VERSION` at the top of the `Makefile` is the single source of truth and **must match
the ONDEWO T2S API in major and minor version**. `make update_composer_version` propagates it into
`composer.json`; never edit that field by hand.

```bash
# bump ONDEWO_T2S_VERSION and the submodule pins, add a RELEASE.md entry, then:
make ondewo_release
```
