...
The client is published on [Packagist](https://packagist.org/packages/ondewo/t2s-client-php) as
**`ondewo/t2s-client-php`** and is installed like any other composer package:

```bash
composer require ondewo/t2s-client-php
```

The client version tracks the ONDEWO T2S API in major and minor version, so pin the minor series that
matches the server you talk to:

```bash
composer require ondewo/t2s-client-php:^6.6
```
...
* **`ext-grpc` is a hard requirement**, not an optional extra — every generated `<Service>Client`
  ...
  (see [Requirements](#requirements) above); `composer require ondewo/t2s-client-php --ignore-platform-req=ext-grpc`
...
* **There is no build step.** Packagist serves the tree of a git tag verbatim and the generated stubs are
  ...
  `composer require`, `vendor/ondewo/t2s-client-php/src/` already holds every stub and the autoloader is
  generated by composer itself. Nothing needs docker, protoc or the `ondewo-t2s-api` submodule.
...
use Ondewo\T2s\Auth\BearerTokenAuthenticator;   // hand-written, from auth/
use Ondewo\T2s\Text2SpeechClient;               // generated, from src/
```
...
1. **Submit the package once.** Log in to [packagist.org](https://packagist.org/) with the ONDEWO account and
   use *Submit* with the repository URL `https://github.com/ondewo/ondewo-t2s-client-php`. Packagist reads
...
| `check_version_agreement` | `ONDEWO_T2S_VERSION`, `composer.json`'s `name` and `version`, the `RELEASE.md` entry and — on a tag — the tag name drifting apart. A tag that disagrees with the `version` field makes Packagist publish the tree under a version nobody tagged |
