...
# =====================================================================================
# ondewo-t2s-client-php - Makefile
#
# The ONDEWO T2S (Text-to-Speech) gRPC client for PHP. The transport surface is
# generated from the .proto definitions of ondewo-t2s-api by the ondewo-proto-compiler's
...
#   make update_submodules       # fetch ondewo-t2s-api + ondewo-proto-compiler
...
# match the ONDEWO T2S API in major and minor version. `make update_composer_version`
...
# ---------------- BEFORE RELEASE ----------------
# 1 - Update Version Number (ONDEWO_T2S_VERSION below)
# 2 - Update RELEASE.md
# 3 - make build
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO T2S PHP Client ${ONDEWO_T2S_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-t2s-client-php"
...
PACKAGIST_PACKAGE=ondewo/t2s-client-php
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-t2s-client-php repo
	pre-commit install
...
TEST: ## Prints some important variables
	...
	@echo "API Pin: \t\t ${ONDEWO_T2S_API_GIT_BRANCH}"
...
build: checkout_defined_submodule_versions build_compiler generate_ondewo_protos fix_generated_ownership update_composer_version ## Build the client: submodules -> compiler image -> stubs -> version bump
	@echo "$(GREEN)[SUCCESS]$(NC) ondewo-t2s-client-php ${ONDEWO_T2S_VERSION} built"
...
#	(${ONDEWO_T2S_API_DIR}) and this package's composer.json from there - it merges
...
generate_ondewo_protos: ## Generate the PHP gRPC client stubs from the .proto definitions into src/
	@test -d ${ONDEWO_T2S_API_DIR}/${PROTOS_TARGET_SUBDIR} \
		|| { echo "$(RED)[ERROR]$(NC) '${ONDEWO_T2S_API_DIR}/${PROTOS_TARGET_SUBDIR}' not found - run 'make update_submodules' first"; exit 1; }
	@echo "$(BLUE)[INFO]$(NC) Generating PHP stubs from ${ONDEWO_T2S_API_DIR}/${PROTOS_TARGET_SUBDIR} ..."
	docker run --rm \
		-v ${shell pwd}:/input-volume \
		-v ${shell pwd}:/output-volume \
		${PROTO_COMPILER_IMAGE} ${ONDEWO_T2S_API_DIR} ${PROTOS_TARGET_SUBDIR}
...
update_composer_version: ## Set ONDEWO_T2S_VERSION as the `version` field of composer.json
	@perl -i -pe 's/^(\s*"version":\s*")[0-9]+\.[0-9]+\.[0-9]+(")/$${1}${ONDEWO_T2S_VERSION}$${2}/' composer.json
	@echo "$(GREEN)[SUCCESS]$(NC) composer.json version set to ${ONDEWO_T2S_VERSION}"
...
# target compares src/ against the ondewo-t2s-api submodule, and CI checks out NO submodules -
...
# (ai_services.proto -> AiServices.php), so the basename is camel-cased before it is looked up.
	@find ${ONDEWO_T2S_API_DIR}/${PROTOS_TARGET_SUBDIR} -type f -name '*.proto' \
...
# has to run AFTER create_release_tag - Packagist crawls what is on GitHub at that moment.
	make publish
	@echo "$(GREEN)[SUCCESS]$(NC) Release Finished - ${PACKAGIST_PACKAGE} ${ONDEWO_T2S_VERSION} is on Packagist"
...
check_release_notes: ## Assert RELEASE.md carries an entry for ONDEWO_T2S_VERSION
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO T2S PHP Client ${ONDEWO_T2S_VERSION}' entry"; \
...
publish: check_packagist_credentials packagist_dry_run packagist_update ## Validate the package and tell Packagist to crawl the new tag (the PHP equivalent of an upload)
	@echo "$(GREEN)[SUCCESS]$(NC) ${PACKAGIST_PACKAGE} ${ONDEWO_T2S_VERSION} published - https://packagist.org/packages/${PACKAGIST_PACKAGE}"
...
packagist_dry_run: composer_validate composer_validate_strict check_version_agreement check_packagist_payload ## Credential-free verification of the whole packaging path (runs in CI on every push)
	@echo "$(GREEN)[SUCCESS]$(NC) Packagist dry run passed - ${PACKAGIST_PACKAGE} ${ONDEWO_T2S_VERSION} is publishable"
...
check_version_agreement: ## Fail unless ONDEWO_T2S_VERSION, composer.json, RELEASE.md and (when HEAD is a tag) the git tag all agree
	...
	if [ "$$version" != "${ONDEWO_T2S_VERSION}" ]; then \
		echo "$(RED)[ERROR]$(NC) composer.json version is '$$version' but ONDEWO_T2S_VERSION is '${ONDEWO_T2S_VERSION}' - run 'make update_composer_version'"; exit 1; fi; \
	grep -qF "Release ONDEWO T2S PHP Client ${ONDEWO_T2S_VERSION}" RELEASE.md \
		|| { echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO T2S PHP Client ${ONDEWO_T2S_VERSION}' entry - the GitHub release would ship empty notes"; exit 1; }; \
	tag="${RELEASE_TAG}"; \
	...
	elif [ "$$tag" != "${ONDEWO_T2S_VERSION}" ]; then \
		echo "$(RED)[ERROR]$(NC) git tag '$$tag' does not match ONDEWO_T2S_VERSION '${ONDEWO_T2S_VERSION}' - Packagist would publish the tag under the wrong version"; exit 1; \
	else \
		echo "$(BLUE)[INFO]$(NC) git tag '$$tag' agrees with ONDEWO_T2S_VERSION"; \
	fi
	@echo "$(GREEN)[SUCCESS]$(NC) ${PACKAGIST_PACKAGE} ${ONDEWO_T2S_VERSION}: version fields agree"
...
#     mistake it exists to catch. Compare the VALUE to ONDEWO_T2S_VERSION, the way rust/ and
...
spc: ## Checks if the Release Branch, Tag and composer.json version already exist
	...
	@if test "$(composer_version)" != "${ONDEWO_T2S_VERSION}"; then \
		echo "-- Test 3: composer.json is at '$(composer_version)', not ${ONDEWO_T2S_VERSION} - run 'make update_composer_version'!!"; exit 1; \
